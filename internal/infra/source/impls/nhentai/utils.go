...
func (n *Nhentai) crawlGallery(ctx context.Context, galleryID string) (*dto.FetchedArtwork, error) {
	url := sourceURLPrefix + galleryID
	docResp, err := n.reqClient.R().SetContext(ctx).Get(url)
	if err != nil {
		return nil, err
	}
	...
	apiResp, err := n.reqClient.R().SetContext(ctx).Get(apiURLPrefix + galleryID)
	if err != nil {
		return nil, err
	}
	...
	return &dto.FetchedArtwork{
		...
		Artist: &dto.FetchedArtist{
			...
			UID:      artistUid,
			Username: artistName,
			Type:     shared.SourceTypeNhentai,
		},
		Tags:       tags,
		SourceURL:  url,
		Pictures:   pictures,
		R18:        true,
		SourceType: shared.SourceTypeNhentai,
	}, nil
}
