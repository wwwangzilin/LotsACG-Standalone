...
func (c *taggerClient) Predict(ctx context.Context, file io.Reader) (map[string]float64, error) {
	resp, err := c.Client.R().SetContext(ctx).SetFileReader("file", "image", file).Post("/predict")
	if err != nil {
		return nil, err
	}
	...
	return predict.Scores, nil
}
func New(host, token string, timeout time.Duration) (*taggerClient, error) {
	client := req.C().
