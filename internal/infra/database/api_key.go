...
func (d *DB) IncreaseApiKeyUsed(ctx context.Context, key string) error {
	return d.db.WithContext(ctx).Model(&entity.ApiKey{}).Where("key = ?", key).Update("used", gorm.Expr("used + ?", 1)).Error
}

func (d *DB) AddApiKeyQuota(ctx context.Context, key string, quota int) error {
	return d.db.WithContext(ctx).Model(&entity.ApiKey{}).Where("key = ?", key).Update("quota", gorm.Expr("quota + ?", quota)).Error
}

func (d *DB) DeleteApiKey(ctx context.Context, key string) error {
	n, err := gorm.G[entity.ApiKey](d.db).Where("key = ?", key).Delete(ctx)
	if err != nil {
		return err
	}
	if n == 0 {
